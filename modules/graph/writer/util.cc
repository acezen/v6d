...
std::shared_ptr<GraphArchive::GraphInfo> generate_graph_info_with_schema(
  ...
  for (const auto& entry : schema.vertex_entries()) {
    ...
    for (const auto& prop : entry.props_) {
      properties.emplace_back(GAR::Property(prop.name, GAR::DataType::ArrowDataTypeToDataType(prop.type), false));
    }
    auto pg = GAR::CreatePropertyGroup(properties, file_type);
    auto vertex_info = GAR::CreateVertexInfo(entry.label, vertex_block_size, {pg});
  ...
  for (const auto& entry : schema.edge_entries()) {
    ...
    for (const auto& relation : entry.relations) {
      auto edge_info = GAR::CreateEdgeInfo(relation.first, entry.label,
                                           relation.second, edge_block_size,
                                           vertex_block_size, vertex_block_size,
                                           true, default_adjacent_lists, {pg});
